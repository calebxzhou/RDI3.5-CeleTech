...
public class SpeakScopeCommand extends RdiCommand {
	...
	private int exec(CommandContext<CommandSourceStack> context) {
		ThreadPool.newThread(()->{
			...
			playerSet.addAll(playersInLevel.stream().map(Player::getStringUUID).toList());
			pidToSpeakPlayersMap.put(pid,playerSet);
			sendChatMessage(player,RESPONSE_SUCCESS,"成功将聊天范围设定为：岛屿内可见。您的聊天消息只有以下玩家可见："+ Arrays.toString(playerSet.stream().map(PlayerUtils::getPlayerByUuid).toArray()));
