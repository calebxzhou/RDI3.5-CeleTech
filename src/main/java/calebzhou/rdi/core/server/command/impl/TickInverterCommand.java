...
public class TickInverterCommand extends RdiCommand {
    ...
    private int exec(CommandContext<CommandSourceStack> cox) {
        ...
        PlayerUtils.sendChatMessage(source,help);
        return 0;
    }
    private int exec2(CommandContext<CommandSourceStack> context) {
        ...
        switch (arg){
            case "intro" -> PlayerUtils.sendChatMessage(source,intro);
    ...
    private void execBlockEntites(CommandSourceStack source) {
        ThreadPool.newThread(()->{
            ...
            PlayerUtils.sendChatMessage(source, "当前全服共有%d个容器进入了延迟tick列表".formatted(instance.getDelayTickListSize()));
    ...
    private void execEntities(CommandSourceStack source) {
        ThreadPool.newThread(()->{
            ...
            PlayerUtils.sendChatMessage(source, "当前全服共有%d个实体进入了延迟tick列表".formatted(instance.getDelayTickListSize()));
            ServerPlayer player = source.getPlayer();
            if(player==null)
                return;
            //TODO 查看附近的被延迟的tick
