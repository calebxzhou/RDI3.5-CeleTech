package calebzhou.rdimc.celestech.model;

public class Island2Error {
    public static final int sourceAlreadyOwnIsland =-100;
    public static final int sourceAlreadyJoinAnyIsland=-101;
    public static final int sourceNotOwnIsland =-102;
    public static final int sourceNotJoinAnyIsland=-103;

    public static final int targetAlreadyOwnIsland=-110;
    public static final int targetAlreadyJoinAnyIsland=-111;
    public static final int targetNotOwnIsland=-112;
    public static final int targetNotJoinAnyIsland=-113;
    
    public static final int sourceEqualsTarget=-120;

}
