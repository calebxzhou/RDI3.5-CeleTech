package calebzhou.rdi.celestech.command;
        ...
        try {
            if(BaseCommand.this instanceof ArgCommand)
                if(StringUtils.isEmpty(arg.getString())){
                    PlayerUtils.sendChatMessage(player,"指令参数不可为空！");
                    return;
                }
            onExecute(player, arg.getString());
        }catch (NullPointerException e){
            PlayerUtils.sendChatMessage(player, PlayerUtils.RESPONSE_ERROR,"找不到指令的目标！");
        }
        catch (Exception e) {
            e.printStackTrace();
            PlayerUtils.sendChatMessage(player, PlayerUtils.RESPONSE_ERROR, e.getMessage());
        }finally {
