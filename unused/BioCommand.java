...
public class BioCommand extends RdiCommand {
    ...
    */
    ...
    private int exec(ServerPlayer player, String biome,int x1,int y1,int z1,int x2,int y2,int z2) {
        ...
        if(bio == null){
            PlayerUtils.sendChatMessage(player, PlayerUtils.RESPONSE_ERROR,"生物群系"+biome+"不存在 ,请您输入生物群系的标识符。详见https://minecraft.fandom.com/wiki/Biome");
            return 1;
        }
        ...
        if(player.experienceLevel<expNeed){
            PlayerUtils.sendChatMessage(player,PlayerUtils.RESPONSE_ERROR,"您的经验不足"+expNeed+"级，无法更改！");
            return 1;
        }
        ...
        PlayerUtils.sendChatMessage(player,PlayerUtils.RESPONSE_SUCCESS,"成功！重载区块后生效。");
        return 1;
    }
