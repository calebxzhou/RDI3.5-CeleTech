...
public class StruCommand extends RdiCommand {
    ...
    private int exec(ServerPlayer player, String namespc) {
        ...
        if(player.experienceLevel<rs.xpNeed){
            PlayerUtils.sendChatMessage(player, PlayerUtils.RESPONSE_ERROR,"经验不足，需要"+rs.xpNeed+"经验");
            return 1;
        }
        ...
        PlayerUtils.sendChatMessage(player,"成功将结构"+rs.id+"写入您所在的区块"+player.getLevel().getChunk(bpos).getPos());
        return 1;
    }
}
